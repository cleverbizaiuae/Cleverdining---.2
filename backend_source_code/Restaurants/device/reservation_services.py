...
def available_tables(restaurant, start, guests, duration_minutes=None, buffer_minutes=0, exclude_id=None):
...
@transaction.atomic
def create_for_device(*, device, reservation_time, guest_no, duration_minutes=None, buffer_minutes=0, **values):
...
@transaction.atomic
def create_for_available_table(*, restaurant, reservation_time, guest_no, duration_minutes=None, buffer_minutes=0, **values):
    duration = int(duration_minutes or default_duration(restaurant))
    end = reservation_time + timedelta(minutes=duration + int(buffer_minutes or 0))
...
def reschedule(reservation, *, reservation_time, guest_no=None, duration_minutes=None, buffer_minutes=None, device=None, allow_reassign=False):
    # Lock only the reservation row. Both foreign keys are nullable for legacy
    # data, and PostgreSQL rejects FOR UPDATE across their outer joins.
    reservation = Reservation.objects.select_for_update().get(pk=reservation.pk)
    schedule_changed = reservation.reservation_time != reservation_time
